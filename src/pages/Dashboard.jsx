...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-grid-pattern pb-12 font-sans selection:bg-primary-container">
      {/* Toast Notification */}
      {toast.show && (
        <div className={`fixed top-20 right-4 z-50 flex items-center gap-2 px-6 py-3 rounded-full shadow-2xl animate-bounce-in ${toast.type === 'success' ? 'bg-secondary-container text-secondary-onContainer' : 'bg-error-container text-error-onContainer'}`}>
              ...
              <div className={`bg-surface p-8 rounded-google shadow-md border border-primary/10 transition-all ${!isOnline ? 'grayscale opacity-70 pointer-events-none' : ''}`}>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-surface-variant p-5 rounded-google border border-surface-outline flex items-center justify-between">
                    ...
                    >
                      <div className={`absolute top-1 left-1 w-6 h-6 rounded-full bg-white shadow-sm transition-all transform ${data.pump_status === 'ON' ? 'translate-x-6' : ''}`}></div>
                    </button>
                  </div>
